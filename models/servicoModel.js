...
servicoSchema.pre('save', async function(next){
    ...
    if(!this.estado) 
        return next(new AppError(ErrorMessage[22].message, 400));
    if(!this.pacote) 
...
const createServiceStatistic = async function(){
    ...
    const qtServiceActivo=await Servico.find({'estado.estadoCode':1}).count()
    const qtServicePendente= await Servico.find({'estado.estadoCode':0}).count() 
    const qtServiceVencido = await Servico.find({'estado.estadoCode':6}).count()//Serviços com prazo de divulgação vencido
    const ocorrenciaCategoria=await Servico.aggregate([{$match:{$or:[{'estado.estadoCode':{$eq:1}},{'estado.estadoCode':{$eq:6}}]}}]).group({_id:"$categoria",storeQt:{$sum:1}}).sort({storeQt:-1})
    const ocorrenciaPacote=await Servico.aggregate([{$match:{$or:[{'estado.estadoCode':{$eq:1}},{'estado.estadoCode':{$eq:6}}]}}]).group({_id:"$pacote",storeQt:{$sum:1}}).sort({storeQt:-1})
    const ocorrenciaFornecedor = await Servico.aggregate([{$match:{$or:[{'estado.estadoCode':{$eq:1}},{'estado.estadoCode':{$eq:6}}]}}]).group({_id:"$fornecedor",storeQt:{$sum:1}}).sort({storeQt:-1})
...
const createFornecedorStatistic = async function(fornecedorId){
    if(fornecedorId){
 const qtTotalService= await Servico.find({fornecedor:fornecedorId}).count();
 const qtActiveService=await Servico.find({fornecedor:fornecedorId, 'estado.estadoCode':1}).count();
 const qtPendenteService=await Servico.find({fornecedor:fornecedorId, 'estado.estadoCode':3}).count();
        await ProviderStatistic.findOneAndUpdate({fornecedor: fornecedorId}, 
