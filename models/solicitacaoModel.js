...
const createStatisticSolicitacao = async function(){
    ...
    const qtSolicitacaoPendente=await Solicitacao.find({'estado.estadoCode':3}).count() 
    const qtSolicitacaoCancelada= await Solicitacao.find({'estado.estadoCode':4}).count() 
    const qtSolicitacaoConcluida= await Solicitacao.find({'estado.estadoCode':5}).count() 
    const ocorrenciaServico=await Solicitacao.aggregate([{$match:{'estado.estadoCode':{$ne:4}}}]).group({_id:"$servico",storeQt:{$sum:1}}).sort({storeQt:-1})
    const ocorrenciaCliente =await Solicitacao.aggregate([{$group:{_id:"$cliente",storeQt:{$sum:1}}}]).sort({storeQt:-1})
...
const createFornecedorStatistic = async function(idServico){
    if(idServico){
        ...
        const fornecedor= storeQuery[0].fornecedor;    
        const arrayServices= await Servico.find({fornecedor:fornecedor}).select('_id');
        const qtsolicitacao =await Solicitacao.find({servico:arrayServices}).count();
        const solicitacoespendentes =await Solicitacao.find({servico:arrayServices,'estado.estadoCode':3}).count()
        const solicitacoesConcluida=await Solicitacao.find({servico:arrayServices, 'estado.estadoCode':5}).count()
            
        await FornecedorStatistica.findOneAndUpdate({fornecedor: fornecedor},
